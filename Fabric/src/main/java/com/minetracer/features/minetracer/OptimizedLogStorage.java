...
public class OptimizedLogStorage {
                    ...
                    if (allLogs != null) {
                        ...
                        for (Map<String, Object> obj : containerList) {
                            ...
                            try {
                                ...
                                ItemStack stack = ItemStack.fromNbt(null, nbt).orElse(ItemStack.EMPTY);  // Using null WrapperLookup for legacy conversion
    ...
    private static class LogEntryJson {
        ...
        LogEntryJson(LogEntry entry) {
            ...
            try {
                this.itemNbt = ((NbtCompound) ItemStack.CODEC.encodeStart(net.minecraft.nbt.NbtOps.INSTANCE, entry.stack).result().orElse(new NbtCompound())).toString();
            } catch (Exception e) {
                this.itemNbt = "{}";
            }
    ...
    private static class ItemPickupDropLogEntryJson {
        ...
        ItemPickupDropLogEntryJson(ItemPickupDropLogEntry entry) {
            ...
            this.pos = entry.pos.getX() + "," + entry.pos.getY() + "," + entry.pos.getZ();
            try {
                this.itemNbt = ((net.minecraft.nbt.NbtCompound) ItemStack.CODEC.encodeStart(net.minecraft.nbt.NbtOps.INSTANCE, entry.stack).result().orElse(new net.minecraft.nbt.NbtCompound())).toString();
            } catch (Exception e) {
                this.itemNbt = "{}";
            }
            this.world = entry.world;
